...
public class RedisTableSink implements UpsertStreamTableSink<Row> {
	...
	public RedisTableSink(String redisIp, int redisPort, String redisValueType, TableSchema tableSchema) {
	...
	public TableSink<Tuple2<Boolean, Row>> configure(String[] fieldNames, TypeInformation<?>[] fieldTypes) {
		return new RedisTableSink(redisIp, redisPort, redisValueType, tableSchema);
	}
	...
	private RedisOutputFormat newFormat() {
		return RedisOutputFormat.builder().setRedisIp(redisIp).setRedisPort(redisPort).setRedisValueType(redisValueType).build();
	}
